// Déconnexion automatique pour inactivité — repris de Sawali (useIdleTimer),
// avec en plus l'exception « tâches en cours » (voir busyTasks.js).
//
// Surveille l'activité (souris, clavier, défilement, toucher). Après
// `idleMinutes` minutes sans activité : `onWarn` 30 s avant, puis `onLogout`.
// Renvoie le compte à rebours pendant l'avertissement.
// Tant qu'une tâche est en cours, rien ne se déclenche : la période
// d'inactivité ne recommence qu'une fois toutes les tâches terminées.
//
// idleMinutes = 0 désactive complètement la minuterie.
import { useEffect, useRef, useState, useCallback } from "react";
import { busyCount, subscribeBusy } from "@/lib/busyTasks";
...
export function useIdleTimer({ idleMinutes, warningSeconds = 30, onWarn, onLogout, enabled = true }) {
  ...
  const reset = useCallback(() => {
    ...
    // Avertissement (reporté si une tâche est en cours)
    warnTimerRef.current = setTimeout(() => {
      if (busyCount() > 0) { reset(); return; }
    ...
    // Déconnexion (jamais pendant une tâche en cours)
  ...
  useEffect(() => {
    ...
    // Début ou fin d'une tâche : la période d'inactivité repart de zéro
    const unsubscribe = subscribeBusy(() => reset());
