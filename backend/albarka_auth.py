"""Auth ALBARKA — email/password + OTP obligatoire + JWT."""
...
async def send_otp_email(to_email: str, full_name: str, code: str) -> bool:
    """Best-effort SMTP send; False when SMTP not configured (dev mode)."""
...
def require_staff():
    async def _checker(user: dict = Depends(get_current_user)) -> dict:
        ...
        if "client" in roles and len(roles) == 1:
            raise HTTPException(status_code=403, detail="Accès réservé au cabinet")
        return user
    return _checker
